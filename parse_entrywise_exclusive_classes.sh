...
## parse_entrywise_exclusive_classes.sh
...
## and proportion of entries in each type of genomic element. unlike parse_entrywise_output.sh,
## this one only puts an entry in a class if it has no overlap with any other class

if [ $# == 1 ]; then
    INFILE=$1
    ## count the exclusive elements in each class
    ## promoter:
    NUM_PROMOTER=`tail -n +2 $INFILE | awk '{if ($4>0 && $6==0 && $8==0 && $10==0) print $0}' | wc -l`
    ## exon:
    NUM_EXON=`tail -n +2 $INFILE | awk '{if ($4==0 && $6>0 && $8==0 && $10==0) print $0}' | wc -l`
    ## intron:
    NUM_INTRON=`tail -n +2 $INFILE | awk '{if ($4==0 && $6==0 && $8>0 && $10==0) print $0}' | wc -l`
    ## repeat:
