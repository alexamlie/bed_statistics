...
def compute_utr_coverage(fp_utr_f, tp_utr_f, promoter_f, exon_f, intron_f, repeat_f, input_f, entrywise_out_f, summary_out_f):
    start = time.clock()
    """
    the main function to compute the coverage of the entries in the bed file.
    """
        ...
        for entry in input_beds:
            ...
            if entry_data[bed_coords['chrom']] == this_chr:
                ...
                while len(cur_fp_utr) > 1 and cur_fp_utr[bed_coords['chrom']] != this_chr:
                    cur_fp_utr = promoters.readline().strip().split('\t')
                while len(cur_tp_utr) > 1 and cur_tp_utr[bed_coords['chrom']] != this_chr:
                    cur_tp_utr = promoters.readline().strip().split('\t')                    
                while len(cur_promoter) > 1 and cur_promoter[bed_coords['chrom']] != this_chr:
                    cur_promoter = promoters.readline().strip().split('\t')
                while len(cur_exon) > 1 and cur_exon[bed_coords['chrom']] != this_chr:
                    cur_exon = exons.readline().strip().split('\t')
...
if __name__=="__main__":
    ...
    parser.add_argument("--fp_utr", help="The optional .bed file containing the 5' UTR loci", default=None)
    parser.add_argument("--tp_utr", help="The optional .bed file containing the 3' UTR loci", default=None)    
    ...
    if pargs.fp_utr and pargs.tp_utr:
        compute_utr_coverage(pargs.fp_utr, pargs.tp_utr, pargs.promoter_bed, pargs.exon_bed, pargs.intron_bed, pargs.repeat_bed, pargs.input_bed, pargs.entrywise_output, pargs.summary_output)
    else:
