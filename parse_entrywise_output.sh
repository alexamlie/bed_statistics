...
## parse_entrywise_output.sh
## alex amlie-wolf 06-15-15
## a script that takes in the entrywise output from entrywise_bed_coverage.py and reports the number
## and proportion of entries in each type of genomic element
...
if [ $# == 1 ]; then
    ...
    ## exon:
    NUM_EXON=`tail -n +2 $INFILE | awk '{if ($6 > 0) print $0}' | wc -l`
    ## intron:
    NUM_INTRON=`tail -n +2 $INFILE | awk '{if ($8 > 0) print $0}' | wc -l`
    ## repeat:
    NUM_REPEAT=`tail -n +2 $INFILE | awk '{if ($10 > 0) print $0}' | wc -l`
    ## intergenic
